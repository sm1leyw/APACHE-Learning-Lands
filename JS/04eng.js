...
const maxHearts = 3;
let totalStages = engQuizData.length;
...
function loadProgression() {
    const saved = localStorage.getItem('magic_eng_progress');
    if (saved) {
        unlockedStages = parseInt(saved);
    } else {
        unlockedStages = 1; // Default
        localStorage.setItem('magic_eng_progress', unlockedStages);
    }
}
...
function generateMap() {
    ...
    engQuizData.forEach((stage, index) => {
...
function loadQuestion() {
    const stage = engQuizData[currentStageIndex];
...
function endStage(isPassed) {
    ...
    if (isPassed) {
        saveProgression(engQuizData[currentStageIndex].id);
        
        endIcon.innerText = "🎉";
        endTitle.innerText = "สุดยอดไปเลย!";
        endMsg.innerText = `คุณผ่านด่าน ${engQuizData[currentStageIndex].name} แล้ว!`;
...
function resetBossVisuals() {
    ...
    document.getElementById('boss-display-name').innerText = engQuizData[9].bossName; // King Nasir
