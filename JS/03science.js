...
const maxHearts = 3;
let totalStages = scienceQuizData.length;
...
function loadProgression() {
    const saved = localStorage.getItem('magic_science_progress');
    if (saved) {
...
function generateMap() {
    ...
    scienceQuizData.forEach((stage, index) => {
...
function startStage(index) {
    ...
    const stage = scienceQuizData[index];
...
function loadQuestion() {
    const stage = scienceQuizData[currentStageIndex];
...
function endStage(isPassed) {
    ...
    if (isPassed) {
        saveProgression(scienceQuizData[currentStageIndex].id);
        
        endIcon.innerText = "🎉";
        endTitle.innerText = "สุดยอดไปเลย!";
        endMsg.innerText = `คุณผ่านด่าน ${scienceQuizData[currentStageIndex].name} แล้ว!`;
...
function resetBossVisuals() {
    ...
    document.getElementById('boss-display-name').innerText = scienceQuizData[9].bossName; // King Nasir
